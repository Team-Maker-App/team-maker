...
const List = () => {
  const players = ["diosito", 'yeray', 'burro', 'German', "Gonzalo", 'Lele', 'Herni', 'Ale', 'Pablo', 'Nico oliva', 'Damián', 'Pleis']
    ...
    if (navigator.share) {
      navigator
        .share({
          ...
          url: 'https://tmssl.akamaized.net/images/portrait/originals/57473-1458043478.jpg',
        })
        .then(() => {
      ...
      <div className="flex justify-around mb-5">
        <div className="w-80 bg-white	min-h-80 mx-3 rounded-md divide-y mr-1">
          <div className="flex justify-center items-center my-2">
            <TShirt />
            <h6 className="my-3 font-display font-bold text-3xl ml-1">Equipo 1</h6>
          </div>
          {firstHalf?.map((player, index) => (
            <p key={index} className="font-display text-2xl h-8 p-1 my-2">{player}</p>
          ))}
        </div>
        ...
        <div className="w-80 bg-white	min-h-80 mx-3 rounded-md divide-y ml-1">
          <div className="flex justify-center items-center my-2">
            <TShirtBlue />
            <h6 className="my-3 font-display font-bold text-3xl ml-1">Equipo 2</h6>
          </div>
          {secondHalf?.map(player => (
            <p className="font-display text-2xl h-8 p-1 my-2">{player}</p>
          ))}
        </div>
      </div>
      <Alert />
